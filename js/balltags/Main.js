...
function reset() {
	...
	var ballNode=document.getElementById('ball-tags').getElementsByTagName('a');

	// console.log(ballNode);
	for( var i=0; i<ballNode.length; i++){
		console.log(ballNode[i].innerHTML);
		createBallTags( ballNode[i].innerHTML , ballNode[i].href);
	}
}
...
function createInstructions() {
	...
	text.innerHTML = '<span style="color:' + theme[0] + ';font-size:40px;"><a href="/" style="text-decoration:none;">首页</a></span><br /><br /><span style="font-size:15px;"><strong>这里是工作说明:</strong><br />1. 点击背景创建小球<br />(可以试着按住不放).<br />2.点击球中非文字区域，可以拖拽.<br />3. 点击文字跳转标签.<br />4. 来玩啊!</span>';
...
function createBallTags( tTitle, tUrl ) {
	var x = x || Math.random() * stage[2];
	var y = y || Math.random() * -200;

	 if (escape(tTitle).indexOf( "%u" )<0)
	 {
		var size = tTitle.length * 20 + 20;
	 }else{//包含有中文
		var size = tTitle.length * 40 + 20;
	 }
	...
	graphics.fillStyle = theme[ (Math.random() * 4 >> 0) + 2];
	graphics.beginPath();
	graphics.arc( size * .5, size * .5, size * .5, 0, PI2, true );
	graphics.closePath();
	graphics.fill();
	...
	text = document.createElement( 'div' );
	text.onSelectStart = null;
	text.innerHTML = '<a href="'+tUrl+'" style="text-decoration:none;"><span style="color:' + theme[0] + ';font-size:40px;">' + tTitle + '</span></a>';
	text.style.color = theme[1];
	text.style.position = 'absolute';
	text.style.left = '0px';
	text.style.top = '0px';
